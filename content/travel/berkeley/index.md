---
title: Berkeley
id: berkeley
geography:
  continent: { id: north-america, label: North America }
  country_or_area: { id: united-states, label: United States }
  admin_path:
    - { kind: state, label: California }
    - { kind: county, label: Alameda County }
  place_kind: { id: city, label: City }
  card_badge: City
coordinates: { lat: 37.8716, lon: -122.2728, zoom: 10 }
trips:
  - id: berkeley-2026-march
    status: visited
    when:
      start: "2026-03-02"
      end: "2026-03-06"
      display: "2–6 March 2026"
      precision: day
    note: I travelled to UC Berkeley for the 5th ACM Symposium on Computer Science and Law (CS&Law 2026), where I presented my work during the poster session. After the conference, I spent my final day exploring the campus.
    photos:
      - file: cslaw-2026-poster-2026-03-04.jpg
        alt: Yixiong Gao standing beside his research poster at CS&Law 2026
        thumbnail_focus: top
        when:
          start: "2026-03-04"
          display: "4 March 2026"
        note: With my poster, “Online Judicial Case Assignment with Quality and Fairness Baselines,” at the 5th ACM Symposium on Computer Science and Law.
        spot:
          name: UC Berkeley
          area: Berkeley, California
      - file: simons-institute-2026-03-06.jpg
        alt: Yixiong Gao outside the Simons Institute for the Theory of Computing at UC Berkeley
        when:
          start: "2026-03-06"
          display: "6 March 2026"
        note: After the conference, I visited the Simons Institute for the Theory of Computing while exploring the UC Berkeley campus.
        spot:
          name: Simons Institute for the Theory of Computing
          area: UC Berkeley
      - file: sather-gate-2026-03-06.jpg
        alt: Sather Gate beneath a clear blue sky at UC Berkeley
        when:
          start: "2026-03-06"
          display: "6 March 2026"
        note: Sather Gate, one of UC Berkeley’s best-known landmarks.
        spot:
          name: Sather Gate
          area: UC Berkeley
      - file: sather-tower-2026-03-06.jpg
        alt: Sather Tower rising above the trees at UC Berkeley
        when:
          start: "2026-03-06"
          display: "6 March 2026"
        note: Sather Tower, the campanile overlooking the UC Berkeley campus.
        spot:
          name: Sather Tower
          area: UC Berkeley
_build: { render: never, list: local, publishResources: false }
---
A conference trip to Berkeley for CS&Law 2026, followed by a day exploring the UC Berkeley campus.
